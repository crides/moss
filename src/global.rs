...
use crate::class::{class_new, Class};
use crate::compiler::Value;
use crate::data::{base16, Bytes};
use crate::iterable::new_iterator;
use crate::iterable::{cycle, iter};
use crate::long::{pow_mod, Long};
use crate::map::map_extend;
use crate::module::eval_module;
use crate::object::{
    downcast, float, new_module, CharString, EnumFunction, FnResult, Function, Info, List, Map,
    Object, Table, VARIADIC,
};
use crate::rand::Rand;
use crate::range::Range;
use crate::system::{open_module_file, History};
use crate::vm::{interface_index, interface_types_set, op_lt, Env, RTE};
...
fn float_to_string(env: &Env, x: &Object, fmt: &Object, precision: &Object) -> FnResult {
    let n = match *precision {
        ...
        _ => {
            return env
                .type_error("Type error in str(x,fmt,precision): precision is not an integer.")
        }
    };
    let fmt = match *fmt {
    ...
    let x = match *x {
        ...
        _ => {
            return env.type_error(
                "Type error in str(x,fmt,precision): x should be of type Float or Int.",
            )
        }
    };
    if fmt.len() != 1 {
    ...
    let s = match fmt[0] {
        ...
        _ => {
            return env.value_error(
                "Value error in str(x,fmt,precision): fmt should be one of 'f', 'e', 'E'.",
            );
        }
    };
...
fn repr(env: &mut Env, _pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        1 => {}
        n => return env.argc_error(n, 1, 1, "repr"),
    }
...
fn sgn(env: &mut Env, _pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        1 => {}
        n => return env.argc_error(n, 1, 1, "sgn"),
    }
    match argv[0] {
        Object::Int(x) => {
            return Ok(Object::Int(match x.cmp(&0) {
                Ordering::Less => -1,
                Ordering::Equal => 0,
                Ordering::Greater => 1,
            }));
        }
        Object::Float(x) => Ok(Object::Float(x.signum())),

        Object::Complex(z) => Ok(Object::Complex(z / z.norm())),

        Object::Interface(ref x) => x.clone().sgn(env),
...
fn abs(env: &mut Env, _pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        1 => {}
        n => return env.argc_error(n, 1, 1, "abs"),
    }
    match argv[0] {
        Object::Int(x) => Ok(Object::Int(x.abs())),

        Object::Float(x) => Ok(Object::Float(x.abs())),

        Object::Complex(z) => Ok(Object::Float(z.norm())),

        Object::Interface(ref x) => x.clone().abs(env),
...
fn eval(env: &mut Env, _pself: &Object, argv: &[Object]) -> FnResult {
    let gtab = match argv.len() {
        ...
        n => return env.argc_error(n, 1, 1, "eval"),
    };
    match argv[0] {
        Object::String(ref s) => {
            let s = s.to_string();
            env.eval_string(&s, "eval", gtab, Value::Optional)
        }
        _ => env.type_error1("Type error in eval(s): s is not a string.", "s", &argv[0]),
    }
}
...
fn len(env: &mut Env, _pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        1 => {}
        n => return env.argc_error(n, 1, 1, "len"),
    }
    match argv[0] {
        Object::List(ref a) => Ok(Object::Int(a.borrow().v.len() as i32)),
        Object::Map(ref m) => Ok(Object::Int(m.borrow().m.len() as i32)),
        Object::String(ref s) => Ok(Object::Int(s.data.len() as i32)),
                    ...
                    if let Object::Int(b) = r.b {
                        if let Object::Null = r.step {
                            ...
                            if let Some(value) = range_length(a, b, step) {
                                ...
                                return env.value_error("Value error in len(a..b: step): step==0.");
                            }
                        }
                    }
                }
...
fn load_file(env: &mut Env, id: &str) -> FnResult {
    if !env.rte().read_access(id) {
        return env.std_exception(&format!(
            "Error in load(id): Could not open file id=='{}': permission denied.",
            id
        ));
    }
    ...
    let value = if binary {
        ...
        if f.read_to_string(&mut s).is_err() {
            return env.std_exception(&format!(
                "Error in load: could not read file '{}.moss'.",
                id
            ));
        }
        env.rte().main_module.set(false);
...
fn fload(env: &mut Env, _pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        1 => {}
        n => return env.argc_error(n, 1, 1, "load"),
    }
    match argv[0] {
...
fn fiter(env: &mut Env, _pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        1 => {}
        n => return env.argc_error(n, 1, 1, "iter"),
    }
...
fn record(env: &mut Env, _pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        1 => {}
        n => return env.argc_error(n, 1, 1, "record"),
    }
    if let Some(t) = downcast::<Table>(&argv[0]) {
        ...
        env.type_error1("Type error in record(x): x is not a table.", "x", &argv[0])
    }
}

fn fobject(env: &mut Env, _pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        0 => Ok(Object::Interface(Table::new(Object::Null))),
        1 => Ok(Object::Interface(Table::new(argv[0].clone()))),
        2 => match argv[1] {
            Object::Map(ref m) => Ok(Object::Interface(Rc::new(Table {
                prototype: argv[0].clone(),
                map: m.clone(),
            }))),
            _ => env.type_error1("Type error in object(p,m): m is not a map.", "m", &argv[1]),
        },
...
fn ftype(env: &mut Env, _pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        1 => {}
        n => return env.argc_error(n, 1, 1, "type"),
    }
...
pub fn list(env: &mut Env, obj: &Object) -> FnResult {
    ...
    if let Some(r) = downcast::<Range>(obj) {
        let a = match r.a {
            ...
            _ => return crate::iterable::to_list(env, obj, &[]),
        };
        let b = match r.b {
            Object::Int(x) => x,
            Object::Float(_) => {
            ...
            _ => {
                return env.type_error1("Type error in list(a..b): b is not an integer.", "b", &r.b)
            }
        };
        let d = match r.step {
            Object::Null => 1,
...
fn copy(env: &mut Env, _pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        1 => {}
        n => return env.argc_error(n, 1, 1, "copy"),
    }
    match argv[0] {
        Object::List(ref a) => Ok(List::new_object(a.borrow().v.clone())),
        Object::Map(ref m) => Ok(Map::new_object(m.borrow().m.clone())),
        ref x => Ok(x.clone()),
    }
}
...
fn rng_range(env: &mut Env, r: &Range) -> FnResult {
    let a = match r.a {
        Object::Int(x) => x,
        _ => return env.type_error1("Type error in rng(a..b): a is not an integer.", "a", &r.a),
    };
    let b = match r.b {
        Object::Int(x) => x,
        _ => return env.type_error1("Type error in rng(a..b): b is not an integer.", "b", &r.b),
    };
    let seed = env.rte().seed_rng.borrow_mut().rand_u32();
    let mut rng = Rand::new(seed);
    let f = Box::new(move |_: &mut Env, _: &Object, _: &[Object]| -> FnResult {
...
fn rng(env: &mut Env, _pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        ...
        1 => {
            ...
            return env.type_error1("Type error in rng(r): r is not a range.", "r", &argv[0]);
        }
        ...
        match argv[0] {
            Object::Function(ref f) => {
                if let EnumFunction::Std(ref fp) = f.f {
                    ...
                    env.type_error(
                        "Type error in gtab(f): f is not a function from Moss source code.",
                    )
                }
            }
            _ => env.type_error1("Type error in gtab(f): f is not a function.", "f", &argv[0]),
        }
    } else {
...
fn string_to_int(env: &mut Env, a: &[char]) -> FnResult {
    match parse_int(a) {
        ...
        Err(e) => match e {
            ParseError::Invalid => {}
            ParseError::Overflow => match Long::object_from_string(a) {
                Ok(value) => return Ok(value),
                Err(()) => {}
            },
        },
    }
    return env.value_error("Value error in int(s): could not convert s into an integer.");
}

fn int(env: &mut Env, _pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        1 => {}
        n => return env.argc_error(n, 1, 1, "int"),
    }
    match argv[0] {
...
fn to_float(env: &mut Env, _pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        1 => {}
        n => return env.argc_error(n, 1, 1, "float"),
    }
    match argv[0] {
        Object::Int(n) => return Ok(Object::Float(float(n))),
        Object::Float(x) => return Ok(Object::Float(x)),
        Object::Complex(z) => return Ok(Object::Float(if z.im == 0.0 { z.re } else { NAN })),
        Object::String(ref s) => {
    ...
    match argv.len() {
        0 => {
            let s = match crate::system::getline("") {
                Ok(s) => s,
                Err(_) => return env.std_exception("Error in input(): could not obtain input."),
            };
            Ok(CharString::new_object_str(&s))
        }
        1 | 2 => {
            ...
            let s = if argv.len() == 2 {
                if let Object::List(ref a) = argv[1] {
                    ...
                    match crate::system::getline_history(&prompt, &h) {
                        Ok(s) => s,
                        Err(_) => {
                            return env.std_exception("Error in input(): could not obtain input.")
                        }
                    }
                } else {
                    return env.type_error1(
...
fn fconst(env: &mut Env, _pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        1 => {}
        n => return env.argc_error(n, 1, 1, "const"),
    }
    match argv[0] {
        Object::List(ref a) => {
...
fn read(env: &mut Env, _pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        1 => {}
        n => return env.argc_error(n, 1, 1, "read"),
    }
    match argv[0] {
        Object::String(ref id) => {
            ...
            if !env.rte().read_access(&id) {
                return env.std_exception(&format!(
                    "Error in read(id): Could not open file id=='{}': permission denied.",
                    id
                ));
            }
        ...
        ref x => env.type_error1("Type error in read(id): id is not a string.", "id", x),
    }
}
...
fn _zip(env: &mut Env, _pself: &Object, argv: &[Object]) -> FnResult {
    ...
    for i in 0..argc {
        if n != v[i].borrow().v.len() {
            return env
                .type_error("Type error in f[a1,...,an]: all lists must have the same size.");
        }
    }
...
fn pow(env: &mut Env, _pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        3 => {}
        n => return env.argc_error(n, 3, 3, "pow"),
    }
...
fn min(env: &mut Env, _pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        2 => {}
        n => return env.argc_error(n, 2, 2, "min"),
    }
...
fn max(env: &mut Env, _pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        2 => {}
        n => return env.argc_error(n, 2, 2, "max"),
    }
    let cond = op_lt(env, &argv[0], &argv[1])?;
    if let Object::Bool(cond) = cond {
...
fn ord(env: &mut Env, _pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        1 => {}
        n => return env.argc_error(n, 1, 1, "ord"),
    }
    match argv[0] {
        Object::String(ref s) => {
            if s.data.len() == 1 {
...
fn chr(env: &mut Env, _pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        1 => {}
        n => return env.argc_error(n, 1, 1, "chr"),
    }
    match argv[0] {
        Object::Int(n) => Ok(match char::from_u32(n as u32) {
...
fn map(env: &mut Env, _pself: &Object, argv: &[Object]) -> FnResult {
    ...
    loop {
        ...
        if y.is_empty() {
            ...
            if a.v.len() != 2 {
                return env
                    .type_error("Type error in map(a): iter(a) is expected to return pairs.");
            }
...
fn long(env: &mut Env, _pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        1 => {}
        n => return env.argc_error(n, 1, 1, "long"),
    }
...
fn getattr(env: &mut Env, _pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        2 => {}
        n => return env.argc_error(n, 2, 2, "getattr"),
    }
...
fn hex(env: &mut Env, _pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        1 => {}
        n => return env.argc_error(n, 1, 1, "hex"),
    }
...
fn bin(env: &mut Env, _pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        1 => {}
        n => return env.argc_error(n, 1, 1, "bin"),
    }
    if let Object::Int(n) = argv[0] {
...
fn oct(env: &mut Env, _pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        1 => {}
        n => return env.argc_error(n, 1, 1, "oct"),
    }
    if let Object::Int(n) = argv[0] {
...
pub fn init_rte(rte: &RTE) {
    ...
    let type_bytes = Class::new("Bytes", &Object::Interface(rte.type_iterable.clone()));
    {
