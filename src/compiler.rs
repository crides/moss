...
use crate::system;
use crate::vm::{bc, Module, BCAASIZE, BCASIZE, BCSIZE, RTE};
...
impl Token {
    fn operator(line: usize, col: usize, value: Symbol) -> Token {
        Token {
            token_type: SymbolType::Operator,
            value,
            col,
            line,
            item: Item::None,
    ...
    fn aoperator(line: usize, col: usize, value: Symbol) -> Token {
        Token {
            token_type: SymbolType::Assignment,
            value,
            col,
            line,
            item: Item::None,
    ...
    while i < n {
        ...
        if c.is_digit(10) {
            ...
            match is_keyword(id) {
                ...
                None => {
                    v.push(Token {
                        token_type: SymbolType::Identifier,
                        value: Symbol::None,
                        line,
                        col: hcol,
...
fn print_token(x: &Token) {
    match x.token_type {
        ...
        SymbolType::Identifier | SymbolType::Int | SymbolType::Float => match x.item {
            Item::String(ref s) => {
                print!("[{}]", s);
            }
            Item::Int(i) => {
...
enum Info {
    ...
    _A(Box<ComplexInfoA>),
    Int(i32),
    Argv { variadic: bool, selfarg: bool },
    Long,
}
...
mod var_tab {
    ...
    impl VarTab {
        ...
        pub fn borrow_parts(&mut self) -> (&[VarInfo], &mut Option<Box<VarTab>>) {
        ...
        pub fn count_optional_arg(&self) -> usize {
            self.count_optional_arg
        }
...
mod pool {
    use crate::object::{CharString, Object};
    ...
    impl Pool {
        ...
        pub fn into_data(self) -> Vec<Object> {
            self.data
        }
...
fn operator(value: Symbol, a: Box<[Rc<AST>]>, line: usize, col: usize) -> Rc<AST> {
    Rc::new(AST {
        line,
        col,
...
fn unary_operator(line: usize, col: usize, value: Symbol, x: Rc<AST>) -> Rc<AST> {
    Rc::new(AST {
        line,
        col,
        symbol_type: SymbolType::Operator,
...
fn binary_operator(line: usize, col: usize, value: Symbol, x: Rc<AST>, y: Rc<AST>) -> Rc<AST> {
    Rc::new(AST {
        line,
        col,
        symbol_type: SymbolType::Operator,
...
fn empty_list(line: usize, col: usize, symbol_type: SymbolType, symbol: Symbol) -> Rc<AST> {
    Rc::new(AST {
        line,
        col,
        symbol_type,
...
impl<'a> Compilation<'a> {
    ...
    fn map_literal(&mut self, i: &mut TokenIterator) -> ResultAST {
        ...
        Ok(Rc::new(AST {
            ...
            info: Info::None,
            s: None,
            a: Some(v.into_boxed_slice()),
        }))
    }

    fn table_literal(&mut self, i: &mut TokenIterator, t0: &Token) -> ResultAST {
        let p = i.next_token(self)?;
        let t = &p[i.index];
        ...
        Ok(Rc::new(AST {
            ...
            info: Info::None,
            s: None,
            a: Some(Box::new([prototype, map])),
        }))
    }

    fn expect_assignment(&mut self, i: &mut TokenIterator) -> Result<(), Error> {
        let p = i.next_token(self)?;
        let t = &p[i.index];
    ...
    fn class_statement(&mut self, i: &mut TokenIterator, t0: &Token) -> ResultAST {
        i.index += 1;
        let id = self.atom(i)?;
    ...
    fn concise_function_literal(&mut self, i: &mut TokenIterator, t0: &Token) -> ResultAST {
        ...
        Ok(Rc::new(AST {
            ...
            info: Info::None,
            s: None,
            a: Some(Box::new([args, x])),
        }))
    }

    fn function_body(&mut self, i: &mut TokenIterator, coroutine: bool) -> ResultAST {
    ...
    fn coroutine_info(&mut self, i: &mut TokenIterator) -> Result<(Info, bool), Error> {
        let p = i.next_token(self)?;
        let t = &p[i.index];
    ...
    fn function_literal(&mut self, i: &mut TokenIterator, t0: &Token) -> ResultAST {
        ...
        let y = Rc::new(AST {
            ...
            info,
            s: id,
            a: Some(Box::new([args, body])),
        });
        Ok(y)
    }

    fn function_statement(&mut self, i: &mut TokenIterator, t0: &Token) -> ResultAST {
        let (info, coroutine) = self.coroutine_info(i)?;
        let p = i.next_token(self)?;
        let t = &p[i.index];
        let id = if t.token_type == SymbolType::Identifier {
    ...
    fn application(&mut self, i: &mut TokenIterator, f: Rc<AST>, terminal: Symbol) -> ResultAST {
        let mut v: Vec<Rc<AST>> = Vec::new();
        ...
        if !v_named.is_empty() {
            let m = Rc::new(AST {
                ...
                info: Info::None,
                s: None,
                a: Some(v_named.into_boxed_slice()),
            });
            v.push(m);
        }
    ...
    fn atom(&mut self, i: &mut TokenIterator) -> ResultAST {
            ...
            || t.token_type == SymbolType::Imag
        {
            ...
            self.syntax_nesting -= 1;
            self.parens -= 1;
        } else if t.value == Symbol::Null || t.value == Symbol::False || t.value == Symbol::True {
            i.index += 1;
            ...
            if t.value == Symbol::PLeft {
                ...
                let y = if t2.token_type == SymbolType::Identifier {
                    ...
                    Rc::new(AST {
                        ...
                        value: Symbol::None,
                        info: Info::None,
                        s: Some(t2.item.assert_string().clone()),
                        a: None,
                    })
                } else {
                    self.atom(i)?
    ...
    fn comparison(&mut self, i: &mut TokenIterator) -> ResultAST {
        ...
        let t = &p[i.index];
        let value = t.value;
        if value == Symbol::Lt || value == Symbol::Gt || value == Symbol::Le || value == Symbol::Ge
        {
            i.index += 1;
            let y = self.range(i)?;
    ...
    fn rec_for_if(&mut self, i: &mut TokenIterator, mut expr: Rc<AST>) -> ResultAST {
    ...
    fn for_expression(&mut self, i: &mut TokenIterator, mut expr: Rc<AST>) -> ResultAST {
        expr = unary_node(
            expr.line,
            expr.col,
            SymbolType::Keyword,
    ...
    fn comma_expression(&mut self, i: &mut TokenIterator, value: Symbol) -> ResultAST {
        let x = self.expression(i)?;
        let p = i.next_token_optional(self)?;
        let t = &p[i.index];
        if t.value == Symbol::Comma {
    ...
    fn while_statement(&mut self, i: &mut TokenIterator) -> ResultAST {
        ...
        Ok(Rc::new(AST {
            ...
            info: Info::None,
            s: None,
            a: Some(Box::new([condition, body])),
        }))
    }

    fn comma_list(&mut self, i: &mut TokenIterator, t0: &Token, first: Rc<AST>) -> ResultAST {
        i.index += 1;
        let mut v: Vec<Rc<AST>> = Vec::new();
        v.push(first);
        loop {
    ...
    fn for_statement(&mut self, i: &mut TokenIterator) -> ResultAST {
        ...
        Ok(Rc::new(AST {
            ...
            info: Info::None,
            s: None,
            a: Some(Box::new([variable, a, body])),
        }))
    }

    fn if_statement(&mut self, i: &mut TokenIterator, t0: &Token) -> ResultAST {
        let mut v: Vec<Rc<AST>> = Vec::new();
        let condition = self.expression(i)?;
        let p = i.next_any_token(self)?;
        let t = &p[i.index];
        ...
        loop {
            ...
            if t.value == Symbol::Elif {
                ...
                return Err(self.syntax_error(t.line, t.col, "expected 'elif, 'else' or 'end'."));
            }
        }
        Ok(Rc::new(AST {
            line: t0.line,
    ...
    fn try_catch_statement(&mut self, i: &mut TokenIterator, t0: &Token) -> ResultAST {
        let mut v: Vec<Rc<AST>> = Vec::new();
        ...
        loop {
            ...
            let c = Rc::new(AST {
                ...
                info: Info::None,
                s: None,
                a: Some(if let Some(x) = expression {
                    Box::new([id, x, cblock])
                } else {
                    Box::new([id, cblock])
                }),
    ...
    fn return_statement(&mut self, i: &mut TokenIterator, t0: &Token, symbol: Symbol) -> ResultAST {
        let p = i.next_any_token(self)?;
        let t = &p[i.index];
    ...
    fn raise_statement(&mut self, i: &mut TokenIterator, t0: &Token) -> ResultAST {
        let x = self.comma_expression(i, Symbol::List)?;
        Ok(Rc::new(AST {
            line: t0.line,
            col: t0.col,
    ...
    fn identifier(&mut self, i: &mut TokenIterator) -> ResultAST {
        let p = i.next_token(self)?;
        let t = &p[i.index];
        if t.token_type == SymbolType::Identifier {
            match t.item {
                Item::String(ref s) => Ok(identifier(s, t.line, t.col)),
                _ => unreachable!(),
            }
        } else {
            Err(self.syntax_error(t.line, t.col, "expected an identifer."))
        }
    }

    fn qualification_assignment(
        &mut self,
        id: &str,
        module: Rc<AST>,
        property: &str,
            ...
            if t2.value == Symbol::Assignment {
                ...
                if t2.token_type == SymbolType::Identifier {
                    ...
                    if t2.value == Symbol::Comma {
                        i.index += 1;
                        continue;
                    } else if t2.value == Symbol::Newline
                        || t2.value == Symbol::Terminal
    ...
    fn use_statement(&mut self, i: &mut TokenIterator, t0: &Token) -> ResultAST {
        let mut v: Vec<Rc<AST>> = Vec::new();
        loop {
            let p = i.next_token(self)?;
            let t = &p[i.index];
    ...
    fn global_statement(&mut self, i: &mut TokenIterator, t0: &Token) -> ResultAST {
        let mut v: Vec<Rc<AST>> = Vec::new();
        loop {
            let x = self.atom(i)?;
            v.push(x);
    ...
    fn assert_statement(&mut self, i: &mut TokenIterator, t0: &Token) -> ResultAST {
        let condition = self.expression(i)?;
        let p = i.next_any_token(self)?;
        let t = &p[i.index];
        if t.value == Symbol::Comma {
    ...
    fn statements(&mut self, i: &mut TokenIterator, last_value: Value) -> ResultAST {
        let mut v: Vec<Rc<AST>> = Vec::new();
    ...
    fn compile_map(&mut self, bv: &mut Vec<u32>, t: &Rc<AST>, byte_code: u8) -> Result<(), Error> {
        let a = ast_argv(t);
        for i in 0..a.len() / 2 {
    ...
    fn compile_if(&mut self, v: &mut Vec<u32>, t: &Rc<AST>, is_op: bool) -> Result<(), Error> {
        let a = ast_argv(t);
        ...
        for i in 0..m {
            ...
            write_i32(
                &mut v[index..index + 1],
                (BCSIZE + len) as i32 - index as i32,
            );
        }
        if a.len() % 2 == 1 {
    ...
    fn compile_while(&mut self, v: &mut Vec<u32>, t: &Rc<AST>) -> Result<(), Error> {
        let index1 = v.len();
        let mut index2 = 0;
        self.jmp_stack.push(JmpInfo {
            start: index1,
            breaks: Vec::new(),
        });
        let a = ast_argv(t);
        ...
        if condition {
            let len = v.len();
            write_i32(
                &mut v[index2..index2 + 1],
                (BCSIZE + len) as i32 - index2 as i32,
            );
        }
    ...
    fn compile_for(&mut self, bv: &mut Vec<u32>, t: &Rc<AST>) -> Result<(), Error> {
        let a = ast_argv(t);
        let iter = apply(
            t.line,
            t.col,
            Box::new([identifier("iter", t.line, t.col), a[1].clone()]),
        );
    ...
    fn compile_try_catch(&mut self, bv: &mut Vec<u32>, t: &Rc<AST>) -> Result<(), Error> {
        ...
        let len = bv.len();
        write_i32(
            &mut bv[index1..index1 + 1],
            (BCSIZE + len) as i32 - index1 as i32,
        );
        ...
        if c.len() == 2 {
            ...
            let len = bv.len();
            write_i32(
                &mut bv[index3..index3 + 1],
                (BCSIZE + len) as i32 - index3 as i32,
            );
    ...
    fn compile_app(&mut self, v: &mut Vec<u32>, t: &Rc<AST>) -> Result<(), Error> {
        let a = ast_argv(t);
    ...
    fn compile_fn(&mut self, bv: &mut Vec<u32>, t: &Rc<AST>) -> Result<(), Error> {
        let a = ast_argv(t);
        ...
        match t.s {
            ...
            None => {
                ...
                push_u32(
                    bv,
                    ((t.col as u32 & 0xffff) << 16) | (t.line as u32 & 0xffff),
                );
        ...
        let argc = if selfarg { argv.len() - 1 } else { argv.len() };
    ...
    fn string_literal(&mut self, s: &str, line: usize, col: usize) -> Result<String, Error> {
    ...
    fn compile_default_argument(&mut self, bv: &mut Vec<u32>, t: &Rc<AST>) -> Result<(), Error> {
        let a = ast_argv(t);
        ...
        let ast = Rc::new(AST {
            ...
            info: Info::None,
            s: None,
            a: Some(Box::new([condition, t.clone()])),
        });
        self.compile_ast(bv, &ast)
    }

    fn arguments(&mut self, bv: &mut Vec<u32>, t: &AST, selfarg: bool) -> Result<(), Error> {
        let a = ast_argv(t);
    ...
    fn compile_variable(&mut self, bv: &mut Vec<u32>, t: &AST) -> Result<(), Error> {
        let key = match t.s {
            Some(ref x) => x,
            None => unreachable!(),
        };
    ...
    fn compile_assignment(
        ...
        if self.function_nesting > 0 {
            match self.vtab.index_type(key) {
                Some((index, var_type)) => match var_type {
                    VarType::Local => {
                    ...
                    VarType::FnId => {
                        return Err(self.syntax_error(t.line, t.col, "cannot assign to function."));
                    }
                },
                None => {
                    if self.coroutine {
    ...
    fn compile_compound_assignment(&mut self, bv: &mut Vec<u32>, t: &AST) -> Result<(), Error> {
        let a = ast_argv(t);
    ...
    fn compile_unpack(&mut self, bv: &mut Vec<u32>, t: &AST) -> Result<(), Error> {
        let a = ast_argv(t);
        for (i, x) in a.iter().enumerate() {
    ...
    fn compile_left_hand_side(&mut self, bv: &mut Vec<u32>, t: &AST) -> Result<(), Error> {
        if t.symbol_type == SymbolType::Identifier {
            ...
            while i < n {
                if a[i + 1].symbol_type == SymbolType::None {
                    ...
                    if a[i].symbol_type == SymbolType::Identifier {
                        ...
                        return Err(self.syntax_error(a[i].line, a[i].col, "expected identifier."));
                    }
            ...
            return Err(self.syntax_error(t.line, t.col, "expected identifier before '='."));
        }
        Ok(())
    }

    fn compile_string(&mut self, bv: &mut Vec<u32>, t: &AST) -> Result<(), Error> {
        let s = match t.s {
            Some(ref x) => x,
            None => unreachable!(),
        };
    ...
    fn compile_long(&mut self, bv: &mut Vec<u32>, t: &AST) -> Result<(), Error> {
        let s = match t.s {
            Some(ref x) => x,
            None => unreachable!(),
        };
    ...
    fn compile_assert(&mut self, bv: &mut Vec<u32>, t: &AST) -> Result<(), Error> {
        let a = ast_argv(t);
    ...
    fn compile_ast(&mut self, bv: &mut Vec<u32>, t: &Rc<AST>) -> Result<(), Error> {
        if t.symbol_type == SymbolType::Identifier {
...
fn asm_listing(a: &[u32]) -> String {
    ...
    while i < a.len() {
        ...
        match op {
            ...
            bc::FLOAT => {
                let x = f64::from_bits(load_u64(&a[BCSIZE + i..BCSIZE + i + 2]));
...
fn compile_token_vector(
    ...
    let mut i = TokenIterator {
        index: 0,
        a: Rc::new(v.into_boxed_slice()),
    };
