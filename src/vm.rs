...
use crate::global::{list, type_name};
use crate::iterable::iter;
use crate::list::cartesian_power;
use crate::long::Long;
use crate::map::{subset, subseteq};
use crate::object::{
    downcast, float, ptr_eq_plain, CharString, Downcast, EnumFunction, Exception, FnResult,
    Function, List, Map, Object, OperatorResult, StandardFn, TypeName, VARIADIC,
};
use crate::rand::Rand;
use crate::range::Range;
use crate::tuple::Tuple;
use crate::{function, global, list, module, string};

use crate::compiler;
use crate::compiler::CompilerExtra;
use crate::system::{getline_history, init_search_paths, History};
    ...
    fn eq(&self, b: &Object) -> bool {
        'r: loop {
            match *self {
                ...
                Object::List(ref x) => {
                    return match *b {
                        Object::List(ref y) => x.borrow().v == y.borrow().v,
                        _ => false,
                    };
                }
                Object::Map(ref x) => {
                    return match *b {
                        Object::Map(ref y) => x.borrow().m == y.borrow().m,
                        _ => false,
                    };
                }
                Object::Function(ref f) => {
                    return match *b {
                        Object::Function(ref g) => Rc::ptr_eq(f, g),
                        _ => false,
                    };
                }
                Object::Info(x) => {
...
fn map_to_string_plain(a: &HashMap<Object, Object>, left: &str, right: &str) -> String {
...
pub fn object_to_string_plain(x: &Object) -> String {
    match *x {
        ...
        Object::List(ref a) => match a.try_borrow_mut() {
            Ok(a) => list_to_string_plain(&a.v),
            Err(_) => "[...]".to_string(),
        },
        Object::Map(ref a) => match a.try_borrow_mut() {
...
pub fn object_to_string(env: &mut Env, x: &Object) -> Result<String, Box<Exception>> {
    Ok(match *x {
        ...
        Object::String(ref s) => s.to_string(),
        Object::List(ref a) => match a.try_borrow_mut() {
            Ok(a) => list_to_string(env, &a.v)?,
            Err(_) => "[...]".to_string(),
        },
        Object::Map(ref a) => match a.try_borrow_mut() {
            Ok(a) => map_to_string(env, &a.m, "{", "}")?,
            Err(_) => "{...}".to_string(),
        },
        Object::Function(ref f) => match f.id {
        ...
        Object::Info(x) => x.to_string(),
        Object::Interface(ref t) => t.clone().to_string(env)?,
    })
}
...
fn operator_neg(env: &mut EnvPart, sp: usize, stack: &mut [Object]) -> OperatorResult {
    ...
    match stack[sp - 1].take() {
        Object::Interface(x) => match x.clone().neg(&mut Env { env, sp, stack }) {
            Ok(y) => {
                if y.is_unimplemented() {
                    Err(env.type_error1_plain(
                        sp,
                        stack,
                        "Type error in -x.",
                        "x",
...
fn string_add(a: &CharString, b: &CharString) -> Object {
    ...
    CharString::new_object(v)
}

fn operator_add(env: &mut EnvPart, sp: usize, stack: &mut [Object]) -> OperatorResult {
    'r: loop {
        match stack[sp - 2] {
            Object::Int(x) => match stack[sp - 1] {
                Object::Int(y) => {
            ...
            Object::Float(x) => match stack[sp - 1] {
                Object::Int(y) => {
            ...
            Object::Complex(x) => match stack[sp - 1] {
                Object::Int(y) => {
        ...
        return match stack[sp - 2].clone() {
            Object::String(a) => match stack[sp - 1].clone() {
                Object::String(b) => {
            ...
            Object::List(a) => match stack[sp - 1].clone() {
                Object::List(b) => {
                    stack[sp - 1] = Object::Null;
    ...
    match stack[sp - 1].take() {
        ...
        a => {
            ...
            if let Some(f) = dispatch_leftover(env, &x, &env.rte.key_add) {
                ...
                Err(env.type_error2_plain(sp, stack, "Type error in x+y.", "x", "y", &x, &a))
            }
        }
    }
}

fn operator_sub(env: &mut EnvPart, sp: usize, stack: &mut [Object]) -> OperatorResult {
    'r: loop {
        match stack[sp - 2] {
            Object::Int(x) => {
        ...
        return match stack[sp - 2].clone() {
            Object::Map(a) => match stack[sp - 1].clone() {
                Object::Map(b) => {
                    ...
                    stack[sp - 1] = Object::Null;
                    stack[sp - 2] = Object::Map(Rc::new(RefCell::new(Map { m, frozen: false })));
                    Ok(())
                }
                _ => {
                    break 'r;
...
fn operator_mul(env: &mut EnvPart, sp: usize, stack: &mut [Object]) -> OperatorResult {
    'list: loop {
        'string: loop {
            'r: loop {
                match stack[sp - 2] {
        ...
        match stack[sp - 2] {
            Object::Int(x) => {
                return match stack[sp - 1] {
                    Object::Int(y) => {
                        if y == 0 {
                            return Err(env.value_error_plain("Value error in x%y: y==0."));
                        }
...
fn operator_band(env: &mut EnvPart, sp: usize, stack: &mut [Object]) -> OperatorResult {
            ...
            Object::Map(ref a) => {
                return match stack[sp - 1].clone() {
                    Object::Map(ref b) => {
                        ...
                        stack[sp - 1] = Object::Null;
                        stack[sp - 2] =
                            Object::Map(Rc::new(RefCell::new(Map { m, frozen: false })));
                        Ok(())
                    }
                    _ => {
                        break 'r;
    ...
    Err(env.type_error2_plain(sp, stack, "Type error in x&y.", "x", "y", &x, &y))
}
...
fn operator_eq(env: &mut EnvPart, sp: usize, stack: &mut [Object]) -> OperatorResult {
    'r: loop {
        ...
        return match stack[sp - 2].clone() {
            Object::String(x) => match stack[sp - 1].clone() {
                Object::String(y) => {
                    stack[sp - 1] = Object::Null;
            ...
            Object::List(x) => match stack[sp - 1].clone() {
                Object::List(y) => {
                    stack[sp - 1] = Object::Null;
            ...
            Object::Map(x) => match stack[sp - 1].clone() {
                Object::Map(y) => {
                    stack[sp - 1] = Object::Null;
            ...
            Object::Function(ref f) => match stack[sp - 1].clone() {
...
fn operator_is(sp: usize, stack: &mut [Object]) -> OperatorResult {
    ...
    match stack[sp - 2].take() {
        Object::Function(ref f) => match stack[sp - 1].take() {
            Object::Function(ref g) => {
                stack[sp - 2] = Object::Bool(Rc::ptr_eq(f, g));
                Ok(())
            }
        ...
        Object::Interface(ref a) => match stack[sp - 1].take() {
    ...
    match stack[sp - 1].take() {
        ...
        Object::String(s) => {
            let c = match key {
                ...
                _ => {
                    return Err(env.type_error1_plain(
                        sp,
                        stack,
...
fn operator_map(sp: usize, stack: &mut [Object], size: usize) -> usize {
    ...
    let mut i = 0;
    while i < size {
        m.insert(stack[sp - size + i].take(), stack[sp - size + i + 1].take());
        i += 2;
    }
    sp -= size;
    ...
    match stack[sp - 2].clone() {
        Object::List(a) => {
            let a = match a.try_borrow() {
                Ok(value) => value,
                Err(_) => {
                    return Err(env.std_exception_plain("Error in a[i]: a is already borrowed."))
                }
            };
                ...
                let index = if i < 0 {
                    ...
                    if iplus < 0 {
                        return Err(env.index_error_plain(&format!(
                            "Error in a[i]: i=={} is out of lower bound.",
                            i
                        )));
                    } else {
                        iplus as usize
        ...
        Object::String(s) => {
            if let Object::Int(i) = stack[sp - 1] {
                let index = if i < 0 {
                    ...
                    if iplus < 0 {
                        return Err(env.index_error_plain(&format!(
                            "Error in s[i]: i=={} is out of lower bound.",
                            i
                        )));
                    } else {
                        iplus as usize
        ...
        Object::Map(m) => match m.borrow().m.get(&stack[sp - 1]) {
            Some(x) => {
                stack[sp - 1] = Object::Null;
                stack[sp - 2] = x.clone();
                Ok(())
...
fn index_assignment(
    ...
    if argc != 1 {
        return match stack[sp - 1 - argc].clone() {
            ...
            _ => {
                return Err(
                    env.type_error_plain("Type error in a[...]=x: got more than one index.")
                );
            }
        };
    }
        ...
        Object::List(a) => {
            ...
            if let Object::Int(i) = index {
                ...
                match a.v.get_mut(index) {
                    ...
                    None => {
                        return Err(env.index_error_plain(&format!(
                            "Error in a[i]: i=={} is out of upper bound.",
                            i
                        )));
                    }
                }
                ...
                match slice_assignment(&mut Env { env, sp, stack }, &mut a.borrow_mut(), &r, &b) {
                    Ok(_) => Ok(()),
                    Err(e) => Err(e),
                }
            } else {
...
#[inline(never)]
fn dispatch_leftover(env: &EnvPart, x: &Object, key: &Object) -> Option<Object> {
...
fn operator_dot(env: &mut EnvPart, sp: usize, stack: &mut [Object]) -> OperatorResult {
    match stack[sp - 2].clone() {
        Object::List(_) => match env.rte.type_list.map.borrow().m.get(&stack[sp - 1]) {
            Some(x) => {
                stack[sp - 2] = x.clone();
                stack[sp - 1] = Object::Null;
                return Ok(());
            }
            None => match env.rte.type_iterable.map.borrow().m.get(&stack[sp - 1]) {
                Some(x) => {
                    stack[sp - 2] = x.clone();
                    stack[sp - 1] = Object::Null;
                    return Ok(());
        ...
        Object::Map(_) => match env.rte.type_map.map.borrow().m.get(&stack[sp - 1]) {
            Some(x) => {
                stack[sp - 2] = x.clone();
                stack[sp - 1] = Object::Null;
                return Ok(());
        ...
        Object::Function(_) => match env.rte.type_function.map.borrow().m.get(&stack[sp - 1]) {
            Some(x) => {
                stack[sp - 2] = x.clone();
                stack[sp - 1] = Object::Null;
                return Ok(());
        ...
        Object::String(_) => match env.rte.type_string.map.borrow().m.get(&stack[sp - 1]) {
            Some(x) => {
                stack[sp - 2] = x.clone();
                stack[sp - 1] = Object::Null;
                return Ok(());
    ...
    return Err(env.index_error_plain(&format!(
        "Index error in t.{0}: '{0}' not in property chain.",
        key
    )));
}
...
fn operator_dot_set(env: &mut EnvPart, sp: usize, stack: &mut [Object]) -> OperatorResult {
    match stack[sp - 2].take() {
        ...
        a => Err(env.type_error1_plain(sp, stack, "Type error in a.x: a is not a table.", "a", &a)),
    }
}

fn operator_get(env: &mut EnvPart, sp: usize, stack: &mut [Object], index: u32) -> OperatorResult {
    'error: loop {
        'clone: loop {
            stack[sp] = match stack[sp - 1] {
                Object::List(ref a) => match a.borrow().v.get(index as usize) {
                    Some(value) => value.clone(),
                    None => {
                        return Err(
                            env.value_error_plain("Value error in unpacking: out of upper bound.")
                        )
                    }
                },
                _ => break 'clone,
    ...
    return Err(env.type_error1_plain(
        sp,
        stack,
        "Type error in x,y = a: a is not a list.",
        "a",
        &a,
    ));
}
...
fn operate(
    ...
    match op as u8 {
        bc::ADD => {
            operator_add(env, sp + 2, stack)?;
        }
        bc::SUB => {
            operator_sub(env, sp + 2, stack)?;
        }
        bc::MUL => {
            operator_mul(env, sp + 2, stack)?;
        }
        bc::DIV => {
            operator_div(env, sp + 2, stack)?;
        }
        bc::IDIV => {
            operator_idiv(env, sp + 2, stack)?;
        }
        bc::BAND => {
            operator_band(env, sp + 2, stack)?;
        }
        bc::BOR => {
            operator_bor(env, sp + 2, stack)?;
        }
        _ => {
            panic!();
        }
...
fn compound_assignment(
    ...
    match key_op as u8 {
        bc::AOP_INDEX => match stack[sp - 3].take() {
            Object::List(a) => {
                let i = match stack[sp - 2] {
                    Object::Int(x) => x,
                    _ => {
                        return Err(
                            env.type_error_plain("Type error in a[i]: i is not an integer.")
                        );
                    }
                };
                let mut a = a.borrow_mut();
                if a.frozen {
                    return Err(
                        env.value_error_plain("Value error in assignment to a[i]: a is frozen.")
                    );
                }
                let index = if i < 0 {
                    let iplus = i + (a.v.len() as i32);
                    if iplus < 0 {
                        return Err(env.index_error_plain(&format!(
                            "Index error in assignment to a[i]: i=={} is out of lower bound.",
                            i
                        )));
                    } else {
                        iplus as usize
                ...
                let p = match a.v.get_mut(index) {
                    ...
                    None => {
                        return Err(env.index_error_plain(&format!(
                            "Index error in assignment to a[i]: i=={} is out of upper bound.",
                            i
                        )));
                    }
                };
            ...
            Object::Map(m) => {
                ...
                if m.frozen {
                    return Err(
                        env.value_error_plain("Value error in assignment to m[key]: m is frozen.")
                    );
                }
                let p = match m.m.get_mut(&key) {
                    ...
                    None => {
                        return Err(
                            env.index_error_plain("Index error in m[key]: key is not in m.")
                        );
                    }
                };
                let x = stack[sp - 1].take();
        ...
        bc::DOT => {
            if let Some(t) = downcast::<Table>(&stack[sp - 3].take()) {
                ...
                if m.frozen {
                    return Err(
                        env.value_error_plain("Value error in assignment to t.(key): t is frozen.")
                    );
                }
                let p = match m.m.get_mut(&key) {
                    Some(value) => value,
                ...
                return Err(
                    env.type_error_plain("Type error in assignment to t.(key): t is not a table.")
                );
            }
        }
        _ => panic!(),
...
fn apply(env: &mut EnvPart, sp: usize, stack: &mut [Object]) -> OperatorResult {
    let f = stack[sp - 3].take();
    let pself = stack[sp - 2].take();
    match stack[sp - 1].take() {
        Object::List(ref a) => match (Env { env, sp, stack }).call(&f, &pself, &a.borrow().v) {
            Ok(y) => {
                stack[sp - 3] = y;
                Ok(())
            }
        ...
        ref a => match list(&mut Env { env, sp, stack }, a) {
            Ok(a) => {
                if let Object::List(ref a) = a {
...
fn mut_fn_aliasing(env: &mut Env, f: &Function) -> Box<Exception> {
    ...
    env.env.std_exception_plain(&format!(
        "Memory error: function '{}' is already borrowed.",
        s
    ))
}
...
impl RTE {
    pub fn new() -> Rc<RTE> {
        ...
        let char_table: Vec<Object> = (0..128).map(|i| Object::from(i as u8 as char)).collect();
    ...
    'main: loop {
        ...
        loop {
            ...
            match a[ip] as u8 {
                ...
                bc::FLOAT => {
                    stack[sp] = Object::Float(f64::from_bits(load_u64(&a, ip + BCSIZE)));
                    sp += 1;
                    ip += BCAASIZE;
                }
                bc::IMAG => {
                    stack[sp] = Object::Complex(Complex64 {
                        re: 0.0,
                ...
                bc::NOT => {
                    let x = match stack[sp - 1] {
                        ...
                        _ => {
                            exception =
                                Err(env
                                    .type_error_plain("Type error in not a: a is not a boolean."));
                            break;
                        }
                    };
                ...
                bc::AND => {
                    let condition = match stack[sp - 1] {
                        ...
                        _ => {
                            exception = Err(
                                env.type_error_plain("Type error in a and b: a is not a boolean.")
                            );
                            break;
                        }
                    };
                ...
                bc::OR => {
                    let condition = match stack[sp - 1] {
                        ...
                        _ => {
                            exception =
                                Err(env
                                    .type_error_plain("Type error in a or b: a is not a boolean."));
                            break;
                        }
                    };
                    if condition {
                ...
                bc::CALL => {
                    ...
                    match fobj {
                        Object::Function(ref f) => match f.f {
                            EnumFunction::Std(ref sf) => {
                                if argc != f.argc as usize {
                                    ...
                                    } else if f.argc_min as usize <= argc
                                        && argc <= f.argc_max as usize
                                    {
                                        ...
                                        exception = Err(
                                            env.argc_error_plain(argc, f.argc_min, f.argc_max, &s)
                                        );
                                        break;
                                    }
                                }
                ...
                bc::LOAD => {
                    ...
                    match gtab.borrow().m.get(key) {
                        ...
                        None => match module.gtab.borrow().m.get(key) {
                            Some(x) => {
                                stack[sp] = x.clone();
                                sp += 1;
                            }
                ...
                bc::TABLE => {
                    sp -= 1;
                    stack[sp - 1] = new_table(stack[sp].take(), stack[sp - 1].take());
                    ip += BCSIZE;
                }
                bc::GET => {
                    let index = load_u32(&a, ip + BCSIZE);
                ...
                bc::RAISE => {
                    sp -= 1;
                    exception = Err(Exception::raise(&env.rte, stack[sp].take()));
                    break;
                }
                bc::AOP => {
...
fn exception_to_string(env: &mut Env, e: &Exception) -> String {
    ...
    if let Some(ref traceback) = e.traceback {
        for x in traceback.v.iter().rev() {
            match x.string(env) {
                Ok(x) => {
                    writeln!(&mut s, "  in {}", x).unwrap();
                }
                Err(e) => {
                    write!(&mut s, "{}", exception_to_string(env, &e)).unwrap();
                    writeln!(
                        &mut s,
                        "[^Another exception occured in str(exception.traceback[k]).]"
                    )
                    .unwrap();
                }
            };
    ...
    if let Some(ref spot) = e.spot {
        writeln!(
            &mut s,
            "Line {}, col {} ({}):",
            spot.line, spot.col, &spot.module
        )
        .unwrap();
    }
...
impl EnvPart {
    ...
    pub fn argc_error_plain(&self, argc: usize, min: u32, max: u32, id: &str) -> Box<Exception> {
        ...
        if min == max {
            if min == 0 {
                Exception::new(
                    &format!("Error in {}: expected no argument, but got {}.", id, argc),
                    t,
                )
            } else if min == 1 {
                Exception::new(
                    &format!("Error in {}: expected 1 argument, but got {}.", id, argc),
                    t,
                )
            } else {
                Exception::new(
                    &format!(
                        "Error in {}: expected {} arguments, but got {}.",
                        id, min, argc
                    ),
                    t,
                )
    ...
    pub fn type_error2_plain(
        ...
        write!(buffer, "\nNote:\n").unwrap();
        {
            ...
            let bsx = match bounded_repr(env, x) {
                Ok(value) => value,
                Err(e) => return e,
            };
            let bsy = match bounded_repr(env, y) {
                Ok(value) => value,
                Err(e) => return e,
            };
            let namex = type_name(env, x);
            let namey = type_name(env, y);
            writeln!(buffer, "  {0}: {1}, {0} = {2},", sx, &namex, &bsx).unwrap();
            write!(buffer, "  {0}: {1}, {0} = {2}.", sy, &namey, &bsy).unwrap();
        }
        self.type_error_plain(&buffer)
    }
}

fn call_object(env: &mut Env, fobj: &Object, _pself: &Object, argv: &[Object]) -> FnResult {
    match *fobj {
        Object::Map(ref m) => {
            match argv.len() {
                1 => {}
                n => return env.argc_error(n, 1, 1, "sloppy index"),
            }
    ...
    return env.type_error1("Type error in f(...): f is not callable.", "f", fobj);
}
...
impl<'a> Env<'a> {
    pub fn call(&mut self, fobj: &Object, pself: &Object, argv: &[Object]) -> FnResult {
        match *fobj {
            Object::Function(ref f) => match f.f {
                ...
                EnumFunction::Mut(ref fp) => {
                    let pf = &mut *match fp.try_borrow_mut() {
                        Ok(f) => f,
                        Err(_) => return Err(mut_fn_aliasing(self, f)),
                    };
    ...
    pub fn command_line_session(&mut self, gtab: Rc<RefCell<Map>>) {
        ...
        loop {
            ...
            let r = compiler::compile(
                ...
                "command line",
                true,
                compiler::Value::Optional,
                &mut history,
                self.rte(),
            );
            dbg!(r.is_ok());
            match r {
                Ok(module) => match eval(self, module.clone(), gtab.clone(), true) {
                    Ok(_) => {}
                    Err(e) => {
    ...
    #[inline(never)]
    pub fn argc_error(&self, argc: usize, min: u32, max: u32, id: &str) -> FnResult {
    ...
    #[inline(never)]
    pub fn type_error2(&mut self, s: &str, sx: &str, sy: &str, x: &Object, y: &Object) -> FnResult {
        return Err(self
            .env
            .type_error2_plain(self.sp, self.stack, s, sx, sy, x, y));
    }
