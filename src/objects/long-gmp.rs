...
impl Mpz {
    ...
    fn from_string(mut s: String) -> Result<Mpz, ()> {
        unsafe {
            let mut mpz = Mpz::new();
            s.push('\x00');
            let base = if s.len() > 3 && (&s[0..2] == "0x" || &s[0..2] == "0b") {
                0
            } else {
                10
            };
    ...
    fn int_sub(&mut self, a: c_long, b: &Mpz) {
        if a < 0 {
            ...
            unsafe {
                __gmpz_ui_sub(&mut self.mpz, a as c_ulong, &b.mpz);
            }
        }
    }
...
impl Long {
    ...
    pub fn from_int(x: i32) -> Long {
        Long {
            value: Mpz::from_int(x.into()),
        }
    }

    pub fn object_from_int(x: i32) -> Object {
        Object::Interface(Rc::new(Long {
            value: Mpz::from_int(x.into()),
        }))
    }
    ...
    pub fn object_from_string(a: &[char]) -> Result<Object, ()> {
        ...
        match Mpz::from_string(s) {
            Ok(y) => Ok(Object::Interface(Rc::new(Long { value: y }))),
            Err(()) => Err(()),
        }
    }
    ...
    pub fn to_long(x: &Object) -> Result<Object, ()> {
        match *x {
            Object::Int(x) => Ok(Long::object_from_int(x)),
            Object::String(ref s) => Long::object_from_string(&s.data),
...
impl Interface for Long {
    ...
    fn sgn(self: Rc<Self>, _env: &mut Env) -> FnResult {
        ...
        return Ok(Object::Int(match s.cmp(&0) {
            Ordering::Less => -1,
            Ordering::Equal => 0,
            Ordering::Greater => 1,
        }));
...
pub fn pow_mod(env: &mut Env, a: &Object, n: &Object, m: &Object) -> FnResult {
    let a = match to_mpz(a) {
        Ok(x) => x,
        Err(()) => {
            return env.type_error("Type error in pow(a,n,m): expected a of type Int or Long.")
        }
    };
    let n = match to_mpz(n) {
        Ok(x) => x,
        Err(()) => {
            return env.type_error("Type error in pow(a,n,m): expected n of type Int or Long.")
        }
    };
    let m = match to_mpz(m) {
        Ok(x) => x,
        Err(()) => {
            return env.type_error("Type error in pow(a,n,m): expected m of type Int or Long.")
        }
    };
    if n.cmp_int(0) < 0 {
