...
use crate::object::{FnResult, List, Map, Object, VARIADIC};
use crate::vm::Env;
...
fn update(env: &mut Env, pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        1 => {}
        n => return env.argc_error(n, 1, 1, "update"),
    }
    match *pself {
        Object::Map(ref m) => match argv[0] {
            Object::Map(ref m2) => {
...
fn extend(env: &mut Env, pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        1 => {}
        n => return env.argc_error(n, 1, 1, "extend"),
    }
    match *pself {
        Object::Map(ref m) => match argv[0] {
            Object::Map(ref m2) => {
...
fn values(env: &mut Env, pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        0 => {}
        n => return env.argc_error(n, 0, 0, "values"),
    }
...
fn items(env: &mut Env, pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        0 => {}
        n => return env.argc_error(n, 0, 0, "items"),
    }
    if let Object::Map(ref m) = *pself {
        let mut index: usize = 0;
        let m = &m.borrow().m;
...
fn clear(env: &mut Env, pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        0 => {}
        n => return env.argc_error(n, 0, 0, "clear"),
    }
    match *pself {
        Object::Map(ref m) => {
...
fn remove(env: &mut Env, pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        1 => {}
        n => return env.argc_error(n, 1, 1, "remove"),
    }
    match *pself {
        Object::Map(ref m) => {
            let mut m = m.borrow_mut();
