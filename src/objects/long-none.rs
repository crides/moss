use crate::object::{FnResult, Object, OperatorResult};
...
impl Long {
    pub fn to_long(_x: &Object) -> Result<Object, ()> {
        Err(())
    }
    pub fn as_f64(&self) -> f64 {
        NAN
    }
    pub fn object_from_string(_a: &[char]) -> Result<Object, ()> {
        Err(())
    }
...
#[inline(never)]
fn overflow_exception(env: &EnvPart, op: &str, x: i32, y: i32) -> OperatorResult {
    Err(env.std_exception_plain(&format!(
        "Integer overflow in x{}y. Note: x={}, y={}.",
        op, x, y
    )))
}
