...
use crate::object::{downcast, Exception, FnResult, Interface, Object};
use crate::vm::{op_eq, Env};
...
impl Interface for Tuple {
    ...
    fn index(self: Rc<Self>, indices: &[Object], env: &mut Env) -> FnResult {
        match indices.len() {
            1 => {}
            n => return env.argc_error(n, 1, 1, "tuple indexing"),
        }
            ...
            if len == b.v.len() {
                for i in 0..len {
                    ...
                    if let Object::Bool(y) = y {
                        ...
                        return env
                            .type_error("Type error in t1==t2: t[i]==t[i] is not a boolean.");
                    }
                }
