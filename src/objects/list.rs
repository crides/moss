...
use crate::object::{FnResult, Function, List, MutableFn, Object, VARIADIC};
use crate::rand::Rand;
use crate::vm::Env;
        ...
        Object::List(ref a) => match a.try_borrow_mut() {
            Ok(mut a) => {
                ...
                if !argv.is_empty() {
                    ...
                    match a.v.pop() {
                        Some(x) => Ok(x),
                        None => env.value_error("Value error in a.pop(): a is empty."),
                    }
                }
            }
            Err(_) => env.std_exception(
...
fn insert(env: &mut Env, pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        2 => {}
        n => return env.argc_error(n, 0, 0, "insert"),
    }
    match *pself {
        Object::List(ref a) => match a.try_borrow_mut() {
            Ok(mut a) => {
                let index = match argv[0] {
                        ...
                        if i < 0 {
                            ...
                            if i < 0 {
                                return env.index_error(
                                    "Index error in a.insert(i,x): i is out of lower bound.",
                                );
                            } else {
                                i as usize
                            }
                    ...
                    ref i => {
                        return env.type_error1(
                            "Type error in a.insert(i,x): i is not an integer.",
                            "i",
                            i,
                        )
                    }
                ...
                if index < a.v.len() {
                    ...
                    return env
                        .index_error("Index error in a.insert(i,x): i is out of upper bound.");
                }
                return Ok(Object::Null);
            }
            Err(_) => {
                return env.std_exception(
                    "Memory error in a.insert(i,x): internal buffer of a is aliased.",
                )
            }
        },
...
fn size(env: &mut Env, pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        0 => {}
        n => return env.argc_error(n, 0, 0, "size"),
    }
    match *pself {
...
fn map(env: &mut Env, pself: &Object, argv: &[Object]) -> FnResult {
    ...
    let a = match *pself {
        ...
        _ => {
            return env.type_error("Type error in a.map(f): a is not a list.");
        }
    };
...
fn filter(env: &mut Env, pself: &Object, argv: &[Object]) -> FnResult {
    ...
    let a = match *pself {
        Object::List(ref a) => a.borrow(),
        _ => {
            return env.type_error("Type error in a.filter(p): a is not a list.");
        }
    };
...
fn new_shuffle() -> MutableFn {
    ...
    return Box::new(
        move |env: &mut Env, pself: &Object, argv: &[Object]| -> FnResult {
            match argv.len() {
                0 => {}
                n => return env.argc_error(n, 0, 0, "shuffle"),
            }
            match *pself {
                Object::List(ref a) => {
...
pub fn map_fn(env: &mut Env, f: &Object, argv: &[Object]) -> FnResult {
    ...
    for k in 0..n {
        for (i, v) in v.iter().take(argc).enumerate() {
            args[i] = v.borrow().v[k].clone();
        }
...
fn clear(env: &mut Env, pself: &Object, argv: &[Object]) -> FnResult {
    match *pself {
        Object::List(ref a) => match a.try_borrow_mut() {
                ...
                match argv.len() {
                    ...
                    1 => {
                        let n = match argv[0] {
                            ...
                            _ => {
                                return env
                                    .type_error("Type error in a.clear(n): n is not an integer.")
                            }
                        };
                        a.v.truncate(n);
                    }
            ...
            Err(_) => {
                env.std_exception("Memory error in a.clear(x): internal buffer of a was aliased.")
            }
        },
...
pub fn cartesian_power(v: &[Object], n: i32) -> Object {
    ...
    return List::new_object(y.into_iter().map(List::new_object).collect());
}
...
fn list_rot(env: &mut Env, pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        1 => {}
        n => return env.argc_error(n, 1, 1, "rot"),
    }
    match *pself {
        Object::List(ref a) => {
            let n = match argv[0] {
                Object::Int(n) => n,
                ref n => {
                    return env.type_error1("Type error in a.rot(n): n is not an integer.", "n", n)
                }
            };
            match a.try_borrow_mut() {
                ...
                Err(_) => {
                    env.std_exception("Memory error in a.rot(n): internal buffer of a was aliased.")
                }
            }
        }
        ref x => env.type_error1("Type error in a.rot(n): a is not a list", "a", x),
    }
}
