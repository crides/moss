...
use crate::object::{downcast, ptr_eq_plain, Exception, FnResult, Interface, List, Map, Object};
use crate::vm::{object_to_string, secondary_env, Env, RTE};
...
fn standard_getter(env: &mut Env, t: Rc<Table>, key: &Object) -> FnResult {
    if let Some(y) = t.map.borrow().m.get(key) {
        return Ok(y.clone());
    } else {
...
fn custom_getter(f: Object) -> PGet {
    Box::new(
        move |env: &mut Env, t: Rc<Table>, key: &Object| -> FnResult {
            env.call(&f, &Object::Interface(t), &[key.clone()])
        },
    )
}

fn standard_setter(_env: &mut Env, t: Rc<Table>, key: Object, value: Object) -> FnResult {
...
fn custom_setter(f: Object) -> PSet {
    Box::new(
        move |env: &mut Env, t: Rc<Table>, key: Object, value: Object| -> FnResult {
            env.call(&f, &Object::Interface(t), &[key, value])
        },
    )
}

fn standard_to_string(env: &mut Env, t: Rc<Table>) -> Result<String, Box<Exception>> {
...
fn custom_to_string(f: Object) -> PToString {
    Box::new(
        move |env: &mut Env, t: Rc<Table>| -> Result<String, Box<Exception>> {
...
pub fn class_new(env: &mut Env, _pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        3 => {}
        n => return env.argc_error(n, 3, 3, "class"),
    }
    let mut drop = None;
...
impl Interface for Table {
    ...
    fn index(self: Rc<Self>, indices: &[Object], env: &mut Env) -> FnResult {
        if let Some(f) = self.slot(&env.rte().key_index) {
            ...
            return env.type_error1(
                "Type error in x[i]: x is not indexable.",
                "x",
    ...
    fn abs(self: Rc<Self>, env: &mut Env) -> FnResult {
        if let Some(f) = self.slot(&env.rte().key_abs) {
            ...
            return env.std_exception("Error: abs(x) is not implemented for objects of this type.");
        }
    }
}
