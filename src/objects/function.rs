...
use crate::object::{FnResult, Function, Object, VARIADIC};
...
fn orbit(env: &mut Env, pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        1 => {}
        n => return env.argc_error(n, 1, 1, "orbit"),
    }
...
fn argc(env: &mut Env, pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        0 => {}
        n => return env.argc_error(n, 0, 0, "argc"),
    }
    if let Object::Function(ref f) = *pself {
        if f.argc == VARIADIC {
            ...
            env.value_error(
                "Value error f.argc(): the count is too large to be represented as i32.",
            )
        } else {
