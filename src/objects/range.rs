...
use crate::iterable::{int_range_iterator, new_iterator};
use crate::object::{
    downcast, float, ptr_eq_plain, CharString, Env, Exception, FnResult, Interface, Object,
};
use crate::vm::{object_to_string, RTE};
...
impl Interface for Range {
    ...
    fn to_string(self: Rc<Self>, env: &mut Env) -> Result<String, Box<Exception>> {
        ...
        Ok(match self.step {
            Object::Null => format!("{}..{}", a, b),
            ref step => format!("{}..{}: {}", a, b, object_to_string(env, step)?),
        })
    }
    ...
    fn rin(&self, key: &Object, env: &mut Env) -> FnResult {
        let k = match *key {
            Object::Int(x) => x,
            _ => {
                return Err(env.type_error_plain("Type error in 'k in i..j': k is not an integer."))
            }
        };
        let i = match self.a {
            Object::Int(x) => x,
            _ => {
                return Err(env.type_error_plain("Type error in 'k in i..j': i is not an integer."))
            }
        };
        let j = match self.b {
            Object::Int(x) => x,
            _ => {
                return Err(env.type_error_plain("Type error in 'k in i..j': j is not an integer."))
            }
        };
        match self.step {
            Object::Null => {}
            _ => {
                return Err(
                    env.type_error_plain("Type error in 'k in i..j: step': step is not supported.")
                )
            }
        }
    ...
    fn iter(self: Rc<Self>, env: &mut Env) -> FnResult {
        let mut a = match self.a {
            ...
            _ => {
                return env.type_error("Type error in iter(a..b): a is not an integer.");
            }
        };
        ...
        let f: Box<dyn FnMut(&mut Env, &Object, &[Object]) -> FnResult> = match self.b {
            ...
            _ => {
                return env.type_error("Type error in iter(a..b): b is not an integer.");
            }
        };
    ...
    fn index(self: Rc<Self>, indices: &[Object], env: &mut Env) -> FnResult {
        match indices.len() {
            1 => {}
            n => return env.argc_error(n, 1, 1, "range index r[...]"),
        }
        match indices[0] {
    ...
    fn is_instance_of(&self, type_obj: &Object, rte: &RTE) -> bool {
        if let Object::Interface(p) = type_obj {
            ptr_eq_plain(p, &rte.type_range) || ptr_eq_plain(p, &rte.type_iterable)
        } else {
            false
        }
    }
...
fn char_range_iterator(env: &mut Env, r: &Range) -> FnResult {
    ...
    let b = if let Object::String(ref s) = r.b {
        if s.data.len() == 1 {
            ...
            return env.value_error("Value error in iter(a..b): b is not a string of size 1.");
        }
    } else {
        return env.type_error("Type error in iter(a..b): b is not of type String.");
    };
    let f = Box::new(
        move |_env: &mut Env, _pself: &Object, _argv: &[Object]| -> FnResult {
            return Ok(if a <= b {
