...
use crate::object::{
    downcast, interface_object_get, new_module, ptr_eq_plain, Exception, FnResult, Interface, List,
    Object,
};
...
impl Bytes {
    pub fn object_from_vec(v: Vec<u8>) -> Object {
        return Object::Interface(Rc::new(Bytes {
            data: RefCell::new(v),
        }));
    }
}
...
impl Interface for Bytes {
    ...
    fn get_type(&self, env: &mut Env) -> FnResult {
        Ok(Object::Interface(
            env.rte().interface_types.borrow()[interface_index::BYTES].clone(),
        ))
    }
    ...
    fn index(self: Rc<Self>, indices: &[Object], env: &mut Env) -> FnResult {
        ...
        match indices.len() {
            1 => {}
            n => return env.argc_error(n, 1, 1, "index operation"),
        }
    ...
    fn add(self: Rc<Self>, b: &Object, env: &mut Env) -> FnResult {
        if let Some(b) = downcast::<Bytes>(b) {
            ...
            return env.type_error1("Type error in a+b: expected b of type Bytes", "b", b);
        }
    }
}
...
fn bytes(env: &mut Env, _pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        1 => {}
        n => return env.argc_error(n, 1, 1, "bytes"),
    }
        ...
        for x in v {
            if let Object::Int(x) = *x {
                if 0 <= x && x < 256 {
                    ...
                    return env
                        .value_error("Value error in bytes(a): a[i] is out of range 0..255.");
                }
            } else {
                return env.value_error("Value error in bytes(a): a[i] is not an integer.");
            }
        }
    ...
    if let Some(bytes) = downcast::<Bytes>(pself) {
        ...
        if spec == "utf-8" {
            ...
            env.value_error(&format!(
                "Value error in a.decode(spec): spec=='{}' is unknown.",
                spec
            ))
        }
    } else {
...
pub fn bytes_len(env: &mut Env, pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        0 => {}
        n => return env.argc_error(n, 0, 0, "len"),
    }
...
pub fn bytes_hex(env: &mut Env, pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        0 => {}
        n => return env.argc_error(n, 0, 0, "hex"),
    }
    if let Some(data) = downcast::<Bytes>(pself) {
...
struct Hash {
    state: RefCell<Option<Sha3_256>>,
}
impl Hash {
    fn new() -> Self {
        Self {
            state: RefCell::new(Some(Sha3_256::new())),
        }
    }
}
...
fn data_hash(env: &mut Env, _pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        0 => {}
        n => return env.argc_error(n, 0, 0, "hash"),
    }
    ...
    if let Some(hash) = downcast::<Hash>(pself) {
        if let Some(data) = downcast::<Bytes>(&argv[0]) {
            ...
            if let Some(state) = state.as_mut() {
                state.update(&data.data.borrow() as &[u8]);
...
fn hash_value(env: &mut Env, pself: &Object, _argv: &[Object]) -> FnResult {
    if let Some(hash) = downcast::<Hash>(pself) {
        ...
        if let Some(state) = state.take() {
            ...
            return Ok(Bytes::object_from_vec(result[..].to_vec()));
        } else {
            return Ok(Object::Null);
        }
    } else {
...
pub fn load_data(env: &mut Env) -> Object {
    ...
    let data = new_module("data");
    {
        ...
        let type_bytes = env.rte().interface_types.borrow()[interface_index::BYTES].clone();
