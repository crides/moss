...
use crate::object::{
    downcast, interface_object_get, new_module, Exception, FnResult, Interface, List, Object,
};
...
fn open(env: &mut Env, _pself: &Object, argv: &[Object]) -> FnResult {
        ...
        2 => match argv[1] {
            ...
            ref x => {
                return env.type_error1(
                    "Type error in open(path,mode): mode is not a string.",
                    "mode",
                    x,
                )
            }
    ...
    let file_id: String = match argv[0] {
        ...
        ref x => {
            return env.type_error1("Type error in open(path): path is not a string.", "path", x)
        }
    };
    ...
    let file = match open_result {
        ...
        Err(_) => {
            return env.std_exception(&format!(
                "Error in open(path): could not open file, path = '{}'.",
                file_id
            ))
        }
    };
...
fn file_write(env: &mut Env, pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        1 => {}
        n => return env.argc_error(n, 1, 1, "write"),
    }
    if let Some(file) = downcast::<File>(pself) {
        ...
        env.std_exception(&format!(
            "Error in f.write(data): failed to write to file '{}'.",
            file.id
        ))
    } else {
...
fn is_file(env: &mut Env, _pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        1 => {}
        n => return env.argc_error(n, 1, 1, "is_file"),
    }
    let file_id: String = match argv[0] {
        Object::String(ref s) => s.to_string(),
...
fn is_dir(env: &mut Env, _pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        1 => {}
        n => return env.argc_error(n, 1, 1, "is_dir"),
    }
    let file_id: String = match argv[0] {
        Object::String(ref s) => s.to_string(),
...
fn read_dir(env: &mut Env, _pself: &Object, argv: &[Object]) -> FnResult {
    let file_id: String = match argv.len() {
        ...
        1 => match argv[0] {
            Object::String(ref s) => s.to_string(),
            _ => return env.type_error("Type error in ls(path): path is not a string."),
        },
    ...
    let it = match path.read_dir() {
        ...
        Err(_) => {
            return env.std_exception(&format!(
                "Error in ls(path): Could not read directory '{}'.",
                file_id
            ))
        }
    };
...
fn change_dir(env: &mut Env, _pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        1 => {}
        n => return env.argc_error(n, 1, 1, "cd"),
    }
...
fn working_dir(env: &mut Env, _pself: &Object, argv: &[Object]) -> FnResult {
    match argv.len() {
        0 => {}
        n => return env.argc_error(n, 0, 0, "wd"),
    }
    let path = match std::env::current_dir() {
        Ok(path) => path,
        Err(_) => {
            return env.std_exception("Error in wd(): could not determine the working directory.")
        }
    };
    if let Some(s) = path.to_str() {
        Ok(Object::from(s))
    } else {
        env.std_exception("Error in wd(): could not encode the working directory as UTF-8.")
    }
}
