...
use crate::object::{FnResult, Map, Object};
use crate::system::History;
use crate::vm::{eval, Env, Module, RTE};
...
fn load_u32(bv: &[u8]) -> u32 {
    bv[0] as u32 | (bv[1] as u32) << 8 | (bv[2] as u32) << 16 | (bv[3] as u32) << 24
}
...
fn load_from_u8(rte: &Rc<RTE>, id: &str, bv: &[u8]) -> Result<Rc<Module>, ()> {
...
fn load_module(rte: &Rc<RTE>, f: &mut File, id: &str) -> Result<Rc<Module>, ()> {
...
pub fn eval_module(env: &mut Env, gtab: Rc<RefCell<Map>>, f: &mut File, id: &str) -> FnResult {
    let m = match load_module(env.rte(), f, id) {
        Ok(m) => m,
        Err(()) => return env.std_exception(&format!("Could not load module '{}'.", id)),
    };
