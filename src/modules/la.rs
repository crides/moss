...
use crate::object::{
    downcast, float, new_module, Exception, FnResult, Interface, List, Object, VARIADIC,
};
use crate::vm::{interface_index, interface_types_set, Env};
...
impl Interface for Array<f64> {
    ...
    fn rmul(self: Rc<Self>, a: &Object, env: &mut Env) -> FnResult {
        let r = match *a {
            ...
            Object::Float(x) => x,
            _ => return env.type_error("Type error in r*v: r has to be of type Int or Float."),
        };
...
fn to_string<T: Number>(a: &Array<T>, _env: &mut Env) -> Result<String, Box<Exception>> {
    if a.n == 1 {
    ...
    if let Some(b) = downcast::<Array<T>>(b) {
        ...
        if a.n == 2 {
            if b.n == 2 {
                ...
                Ok(Object::Interface(mul_matrix_matrix(
                    m, p, n, a, b, adata, bdata,
                )))
            } else if b.n == 1 {
                let m = a.s[0].shape;
                let n = a.s[1].shape;
                if n != b.s[0].shape {
                    return env.value_error("Value error in A*b: A.shape[1] != b.shape[0].");
                }
                Ok(Object::Interface(mul_matrix_vector(
                    m, n, a, b, adata, bdata,
                )))
            } else {
                panic!()
            ...
            let n = a.s[0].shape;
            if n != b.s[0].shape {
                return env.value_error("Value error in a*b: a.shape[0] != b.shape[0].");
            }
...
fn map_binary<T: Number>(a: &Array<T>, adata: &[T], b: &Array<T>, f: &dyn Fn(T, T) -> T) -> FnResult
where
    Array<T>: Interface,
{
    if a.n == 1 {
...
fn map_unary<T: Number>(a: &Array<T>, data: &[T], f: &dyn Fn(T) -> T) -> FnResult
where
    Array<T>: Interface,
{
    if a.n == 1 {
...
fn matrix_from_lists_c64(m: usize, n: usize, env: &mut Env, argv: &[Object]) -> FnResult {
    ...
    for x in argv {
        ...
        if data.len() != n {
            return env
                .value_error("Value error in matrix(*a): all a[i] must be of the same size.");
        }
        for x in data {
            let x = match *x {
                Object::Int(x) => c64 {
...
fn matrix_from_lists(env: &mut Env, argv: &[Object]) -> FnResult {
    let m = argv.len();
    if m == 0 {
        return env.value_error("Value error in matrix(*a): expected at least one row.");
    }
    let n = match argv[0] {
        Object::List(ref a) => {
            let a = &a.borrow().v;
